...
const mysqlHealth = require('./mysql/repos/health');

const VALID_SOURCES = new Set(['mongo', 'mysql']);
...
function resolveSource(domain, overrides = {}) {
  ...
  if (!source) throw new Error('DATA_SOURCE is not set (expected "mongo" or "mysql").');
...
const BUILDERS = {
  health: {
    ...
    mysql: mysqlHealth.createHealthRepo,
  },
};
 ...
 * so hybrid deployments (some domains on Mongo, others on MySQL) work.
 *
